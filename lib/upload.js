...
module.exports = function (program) {
  ...
  blobService.createContainerIfNotExists(program.container, { publicAccessLevel : 'blob' }, function(error){
    ...
    if (program.from) {
      ...
      return blobService.copyBlob(blobUrl, program.container, program.file_to_upload, function(error){
        if(error) return emitter.emit('error', error);
        
        if (!program.quiet) console.log('uploaded!');
    ...
    var size = fs.statSync(program.file_to_upload).size;
    var chunkSize = Math.pow(1024,2) * program.chunksize;
    var chunks = Math.ceil(size / chunkSize);
    var blobName = path.basename(program.file_to_upload);
    ...
    async.timesSeries(chunks, function (n, next) {
      ...
      var stream = fs.createReadStream(program.file_to_upload, {start: start, end: end});
